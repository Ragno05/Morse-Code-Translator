...
while esecuzione==True: #il ciclo che sta al cuore del programma, quello che fa ripetere il tutto il loop
    ...
    if tipo.lower() == "a": #in base alla scelta, va a scegliere se tradurre dal morse o in morse
        da_morse() #avvia la funzione dal morse, e prende la variabile esecuzione restituita dalla funzione precedente
    else:
        per_morse() #avvia la funzione per il morse, e come l'altra, riceve la variabile esecuzione
    esecuzione=finale()