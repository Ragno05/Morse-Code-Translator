...
while esecuzione==True: #the "heart" of the programm, it is a loop which keep going until user tell it to stop
    ...
    if tipo.lower() == "a": #this if define is wants the user to translate from or to morse code
        da_morse() #goes to the function that translates FROM morse
    else:
        per_morse() #and here goe to the function that translates TO morse
    esecuzione=finale() #goes to teh function that ask to the user if he has done with the program